...
class EnhancedTimeSeriesSplit(BaseCrossValidator):
    """Enhanced Time Series Split cross-validator.
    CVTT is a method for more detailed comparisons using temporal model performance.
    Performs hierarchical k-fold cross-validation that can contain validation sets for time series data.

    author:Shangtingyu <shangtingyu1@gmail.com>
    ...
    max_train_size : int, optional
        Maximum size of the training set. If None, all samples before the test set will be used.
    return_validate : bool, optional
        Whether a validation set is required, which defaults to True.
    random_state : int or numpy.random.RandomState, optional
        Random initial state. When shuffle is True, random_state determines the initial ordering of the samples,
        through which the randomness of the selection of various data samples in each compromise can be controlled.
        See sklearn.model_selection.TimeSeriesSplit for details. The default is None.
    ...
    def __init__(self, n_splits: int = 5, max_train_size: Optional[int] = None, return_validate: bool = True,
                 random_state: Optional[Union[int, RandomState]] = None):
        ...
        if self.return_validate:
            test_size = 1 / n_splits
            self.validate_splitter = StratifiedShuffleSplit(n_splits=1, test_size=test_size, random_state=random_state)
    ...
    def split(self, X, y=None, groups=None):
        ...
        for i in range(self.n_splits):
            ...
            if self.return_validate:
                train_ind, validate_ind = next(self.validate_splitter.split(X[train_indices], y[train_indices]))
                yield train_indices[train_ind], train_indices[validate_ind], test_indices
            else:
